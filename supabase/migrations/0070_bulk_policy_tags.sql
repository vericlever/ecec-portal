-- 0070_bulk_policy_tags.sql
--
-- Same gap as 0069, on the policy side: commit_bulk_policies didn't write
-- document_quality_areas / document_child_safe_standards, even though a
-- single-policy edit already tags both (policy-editor.tsx), and the quality
-- area report already reads tags for document_type 'policy' alongside 'sop'
-- (src/lib/reports.ts). p_items gains the same two optional arrays per item:
-- quality_area_ids (int[]), child_safe_standard_ids (int[]).
