-- 0072_bulk_sop_needs_review.sql
--
-- Carries bulk_upload_staging.needs_review (migration 0071) through to the
-- documents row commit_bulk_sops creates, so the flag survives past the
-- staging row's lifetime rather than being visible only during the Step 47
-- review screen. Only the two `insert into public.documents` calls change
-- from 0069 - everything else is identical.
